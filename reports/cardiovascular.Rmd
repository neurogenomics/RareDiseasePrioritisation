...
Search results for anything cardiovascular-related, 
either at the level of phenotype or disease.
...
```{r}
map <- MultiEWCE:::get_data("DescartesHuman_celltype_mapping.csv")
map[,celltype:=EWCE::fix_celltype_names(celltype,make_unique = FALSE)]

ct <- MultiEWCE::terminal_celltypes()
ct[CellType %in% map[tissue=="Heart"]$celltype] 
heart_specific_cells <- map[tissue=="Heart" & !celltype %in% map[tissue!="Heart"]$celltype]
```
...
#### [See here for full-screen version](https://neurogenomics.github.io/RareDiseasePrioritisation/networks/cardiovascular_hierarchical_network.html)
