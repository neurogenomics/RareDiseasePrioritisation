...
```{r}
all_agg <- MultiEWCE::agg_results(phenos = all_targets,
                                  count_var = "CellType", 
                                  group_var = "Phenotype")
...
```{r}
#### All ontology level ####
head(sort(table(unique(all_targets[,c("Phenotype","CellType")])$Phenotype), 
          decreasing = TRUE))

#### Only lower ontology levels #####
all_targets <- HPOExplorer::add_ont_lvl(all_targets)
head(sort(table(unique(all_targets[ontLvl<=4,c("Phenotype","CellType")])$Phenotype), 
          decreasing = TRUE))
```
...
```{r}
head(sort(table(unique(all_targets[,c("Phenotype","CellType")])$CellType), 
          decreasing = TRUE))
```
...
```{r}
head(sort(table(unique(all_targets[,c("Phenotype","Gene")])$Gene), 
          decreasing = TRUE))
```
...
```{r}
ancestor_freq <- sort(table(all_targets$ancestor), decreasing = TRUE) |>
