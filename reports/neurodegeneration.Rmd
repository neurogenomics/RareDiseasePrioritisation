...
pd_targets <- data.table::merge.data.table(pd_specific3,
                             genes[,-c("Phenotype")],
                             by.x = c("HPO_ID","DatabaseID"),
                             by.y = c("HPO_ID","LinkID"))
pd_vn <- MultiEWCE::prioritise_targets_network(top_targets = pd_targets, mediator_var = list(c(1,2),c(2,3),c(3,4),c(2,4),c(1,4)))
```
